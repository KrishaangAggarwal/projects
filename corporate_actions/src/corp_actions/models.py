...
class BreakExplanation:
    ...
    break_type: str = "quantity"  # "quantity", "cash", "ticker"


@dataclass
class ReplayResult:
    """Result of replaying corporate actions on a position."""
    ticker: str = ""
    ...
    total_cash_in_lieu: Decimal = Decimal(0)


@dataclass
class ReconMatch:
    """A single reconciliation comparison result."""
    ticker: str = ""
    internal_qty: Decimal | None = None
    external_qty: Decimal | None = None
    status: str = "UNEXPLAINED"  # MATCHED, EXPLAINED, POSSIBLE, UNEXPLAINED, MISSING
    explanations: list[BreakExplanation] = field(default_factory=list)
    matched_ticker: str | None = None  # if ticker was renamed
    notes: str = ""
