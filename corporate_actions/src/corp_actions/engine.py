...
from corp_actions.models import (
    BreakExplanation, CorporateAction, PositionImpact, ReplayResult, ReconMatch,
)
...
class CorporateActionsEngine:
    ...
    def __init__(
        ...
        # Initialize identifier tracker for break explainer
        try:
            conn = get_db(self._db_path)
            self._identifier_tracker = IdentifierTracker(conn)
        except Exception:
            self._identifier_tracker = None

        self.explainer = BreakExplainer(
            self.split_detector, self.edgar_detector, self._identifier_tracker
        )
    ...
    def explain_all_breaks(
        ...
        """Return ALL possible explanations for a quantity break, ranked."""
        return self.explainer.explain_all_breaks(
            ticker,
            Decimal(str(internal_qty)),
            Decimal(str(external_qty)),
            as_of_date,
    ...
    def explain_cash_break(
        ...
        cash_difference: float | Decimal,
        as_of_date: date | None = None,
        lookback_days: int = 365,
    ) -> list:
        """Explain a cash discrepancy — missed dividends, merger cash, etc."""
        return self.explainer.explain_cash_break(
            ticker,
            Decimal(str(shares)),
            Decimal(str(cash_difference)),
            as_of_date,
            lookback_days,
        )

    def explain_missing_ticker(self, ticker: str) -> list:
        """Explain why a ticker might be missing — renames, delistings, mergers."""
        return self.explainer.explain_missing_ticker(ticker)
    ...
    def replay_position(
        ...
        """Replay all corporate actions on a position from entry to today."""
        replay = PortfolioReplay(self.split_detector)
        return replay.replay_position(
            ticker, Decimal(str(quantity)), entry_date, end_date
        )
    ...
    def replay_portfolio(
        ...
        """Replay corporate actions on a portfolio of positions."""
        replay = PortfolioReplay(self.split_detector)
        return replay.replay_portfolio(positions, end_date)
    ...
    def reconcile(
        ...
        """Reconcile two position sets and explain discrepancies."""
        recon = BulkRecon(self.explainer, self._identifier_tracker)
        return recon.reconcile(internal, external, lookback_days)

    # --- Validation ---

    def validate_action(self, action: CorporateAction) -> CorporateAction:
        """Cross-validate a corporate action against other data sources."""
        validator = CrossValidator(self.split_detector, self.edgar_detector)
        return validator.validate(action)

    def validate_actions(
        self, actions: list[CorporateAction]
    ) -> list[CorporateAction]:
        """Cross-validate a batch of actions."""
        validator = CrossValidator(self.split_detector, self.edgar_detector)
        return validator.validate_batch(actions)
    ...
    def check_for_new_events(
        ...
        """Check tickers once for new events not previously seen."""
        watcher = EventWatcher(
            self.split_detector, self.edgar_detector, self._db_path
        )
        return watcher.check_once(tickers)
    ...
    def watch(
        ...
        """Continuously monitor tickers for new events. Ctrl+C to stop."""
        watcher = EventWatcher(
            self.split_detector, self.edgar_detector, self._db_path
        )
        watcher.watch(tickers, interval, callback)
    ...
    def health_check(self) -> list[dict]:
        ...
        checker = HealthChecker(self._db_path)
